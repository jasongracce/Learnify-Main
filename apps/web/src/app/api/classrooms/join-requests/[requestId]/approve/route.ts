...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    // Verify the approver owns the classroom
    ...
    if (!classroom || classroom.owner_membership_id !== auth.membership.id) {
      return NextResponse.json(
        { error: "Only the classroom owner can approve join requests." },
        { status: 403 }
      )
    }

    const { request: approved, activated } = await approveJoinRequest({
      supabase: auth.serviceSupabase,
      requestId,
    ...
    await insertAuditEvent({
      ...
      targetId: requestId,
      metadata: { activated, status: approved.status },
    })

    // Teacher-facing message — no overage details exposed
    const message =
      approved.status === "approved"
        ? "Approved. Student added."
        : "Approval pending. Your school has reached its student account limit."

    return NextResponse.json({ joinRequest: approved, message })
  } catch (error) {
    console.error("approveJoinRequest failed", {
      error: error instanceof Error ? error.message : error,
    })
    return NextResponse.json({ error: "Could not approve join request." }, { status: 500 })
  }
}
