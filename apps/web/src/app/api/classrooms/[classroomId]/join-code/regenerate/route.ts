  ...
  try {
    ...
    if (classroom.owner_membership_id !== auth.membership.id) {
      return NextResponse.json(
        { error: "Only the classroom owner can regenerate the join code." },
        { status: 403 }
      )
    }
    ...
    await insertAuditEvent({
      ...
      eventType: "classroom.join_code_regenerated",
    ...
    return NextResponse.json({ error: "Could not regenerate join code." }, { status: 500 })
  }
}
